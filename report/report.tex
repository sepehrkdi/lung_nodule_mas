...
The system operates on the IU/Open-I Indiana University Chest X-ray Collection~\cite{demsar2005openi}, which contains approximately 7,470 chest X-ray images paired with free-text radiology reports. This subset size is consistent with prior clinical NLP validation studies. For instance, Chapman et al.\ evaluated the NegEx negation-detection algorithm against a set of \hl{1000 sentences} containing 1235 clinical findings and diseases extracted from discharge summaries~\cite{chapman2001negex}. To focus the evaluation on relevant pathology, I implemented an NLP-based pre-filter pipeline:

\begin{enumerate}
    \item \textbf{Ingestion:} The \texttt{NLMCXR\_Loader} scans all 7,470 reports in the dataset.
    ...
    \item \textbf{Negation Check:} A NegEx-style (Section~\ref{subsec:negex}) rule filter excludes reports where these keywords appear only in negated contexts (e.g., ``no nodule'', ``without mass'').
    \item \textbf{Ground-Truth Filtering:} Each candidate report is passed through the NLP-based ground-truth extractor (Section~\ref{subsec:groundtruth}), which assigns a binary label: 1 (Abnormal/Suspicious) or 0 (Normal/Benign). Reports for which the extractor returns $-1$ (Indeterminate) are excluded.
    \item \textbf{Subset Selection:} The first 500 cases with a definite ground-truth label (either 0 or 1) are selected to form the \emph{Evaluation Subset}, ensuring that both Normal and Abnormal classes are represented and every case contributes to the binary confusion matrix.
\end{enumerate}

This filtering process ensures that the multi-agent system is evaluated exclusively on cases with unambiguous NLP-derived ground truth, providing a balanced binary evaluation set.
...
The system deploys three distinct Radiologist agents to simulate clinical inter-reader variability. Each radiologist agent utilizes the \texttt{TorchXRayVision} library~\cite{cohen2022xrv}, specifically employing the DenseNet-121 architecture proposed in CheXNet~\cite{rajpurkar2017chexnet}. CheXNet has been shown to exceed effective radiologist performance on pneumonia detection tasks, providing a strong baseline for automated chest X-ray reading. The agents specifically monitor the model's output probability for the ``Nodule'' or ``Mass'' pathology classes. No additional CV training is performed; I rely on these robust frozen weights. The system operates in \emph{strict mode}: the CNN model must be available and functional---no heuristic fallbacks are provided. If the model fails to load, a \texttt{ModelUnavailableError} is raised, ensuring classification integrity. The three agents differ in their operating thresholds:
