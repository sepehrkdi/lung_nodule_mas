...
Weighted F1 Score & --- \\
\midrule
Majority Agreement & 96.0\% \\
Split Decisions & 4.0\% \\
\midrule
Mean Consensus Probability & --- \\
Mean Confidence & --- \\
\bottomrule
\end{tabular}
\caption{Multi-agent binary classification results on the 50-case NLMCXR Evaluation Subset. The confusion matrix is a $2 \times 2$ Normal vs.\ Abnormal matrix. ``---'' indicates values to be filled from the dashboard evaluation run.}
\label{tab:results}
\end{table}

\subsubsection{Agent Agreement Analysis}

The 96\% majority agreement rate indicates that at least 4 of 6 agents (3 radiologists + 3 pathologists) reached the same binary classification in most cases. Split decisions (4\%) occurred when radiologist agents disagreed due to different operating thresholds---specifically, the Sensitive radiologist (R3) sometimes classified cases differently from the Conservative (R1) and Balanced (R2) variants. The absence of unanimous agreement (0\%) reflects the deliberate design choice of using diverse agent strategies: in no case did all six agents produce identical probability outputs, although their binary predictions aligned via majority voting in 96\% of cases.
...
The architecture deploys seven agent instances (three radiologists, three pathologists, one oncologist) that communicate via FIPA-compliant message passing. Evaluation on the 50-case Evaluation Subset from the IU/Open-I NLMCXR dataset demonstrated 96\% majority agreement among agents in binary (Normal vs.\ Abnormal) classification, with the consensus mechanism successfully resolving split decisions through weighted voting and conflict detection strategies.
