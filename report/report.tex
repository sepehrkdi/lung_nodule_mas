...
The Pathologist agents are the main NLP contribution, with three active agents analyzing textual evidence. The 3rd agent (Pathologist-3), previously optional, is now enabled to handle negation and uncertainty. The term ``Pathologist'' is used metaphorically to represent agents that analyze textual evidence, analogous to how pathologists extract findings from specimens. Each agent implements a different NLP strategy:
...
Following the principles of collective intelligence~\cite{wolf2015collective}, disagreement is detected when $\sigma > 0.08$ (lowered from 0.15 to increase sensitivity to inter-modality conflicts). The resolution strategies are inspired by multi-agent conflict resolution taxonomies~\cite{atoms2025conflictresolution}, focusing on voting and arbitration. Four resolution strategies are applied in order:

\begin{enumerate}
    \item \textbf{Visual--Text Conflict:} When CNN agents confidently predict malignancy ($P > 0.65$) but NLP agents confidently predict benign ($P < 0.35$), the system identifies a potential visual false positive or a finding not yet recorded in the report. It resolves this by averaging the probabilities but significantly lowering the confidence to 0.4 and triggering a specific ``Radiology Review Required'' recommendation.
    \item \textbf{Text Override (Missed Visual):} When NLP agents confidently predict malignancy ($P > 0.65$) but CNN agents miss it ($P < 0.35$), the system assumes a visual false negative. It trusts the textual ground truth, overrides the visual output with the Pathologist's probability, and assigns high confidence (0.8).
    \item \textbf{Pathologist Override (Indeterminate Visual):} When Pathologists detect malignancy ($P \ge 0.60$) but Radiologists are indeterminate ($0.35 \le P \le 0.65$), the system overrides the weighted average with the Pathologist's probability to prevent dilution of strong textual evidence by uncertain imaging models.
    ...
    \item \textbf{Conservative Default:} Under strong disagreement not covered by specific rules, the system defaults to a conservative recommendation and flags the case for multidisciplinary review.
\end{enumerate}
...
A key limitation of static agent weights is the implicit assumption that every case offers the same quality and quantity of information in both modalities. In practice, some cases are accompanied by multiple high-quality X-ray projections but only a terse report, while others contain detailed reports but limited or low-quality imaging. Static weights cannot adapt to this per-case asymmetry.
...
After a definitive diagnosis is confirmed (e.g., via biopsy results or expert consensus), the system updates the base weights stored in \texttt{data/learned\_weights.json} using a simple reinforcement update rule:
\begin{equation}
