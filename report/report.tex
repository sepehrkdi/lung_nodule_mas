...
The system operates on the IU/Open-I Indiana University Chest X-ray Collection~\cite{demsar2005openi}, which contains approximately 7,470 chest X-ray images paired with free-text radiology reports. This subset size is consistent with prior clinical NLP validation studies, for instance, Chapman et al.\ used 42 reports for NegEx evaluation~\cite{chapman2001negex}. To focus the evaluation on relevant pathology, I implemented an NLP-based pre-filter pipeline:
...
\begin{enumerate}
    ...
    \item \textbf{Subset Selection:} The first 50 cases with a definite ground-truth label (either 0 or 1) are selected to form the \emph{Evaluation Subset}, ensuring that both Normal and Abnormal classes are represented and every case contributes to the binary confusion matrix.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{NLP-Level Ground Truth (Extraction):} For the 50-case Evaluation Subset, token-level manual annotations were created to evaluate the information extraction performance of the Pathologist agents.
\end{itemize}
...
The complete pipeline was evaluated on the 50-case Evaluation Subset from the NLMCXR dataset. Each case was processed by all 6 agents (3 radiologists + 3 pathologists), with the Oncologist computing a binary consensus decision (Normal vs.\ Abnormal). The system-level ground truth was derived from the NLP-based report analysis (Section~\ref{subsec:groundtruth}). Table~\ref{tab:results} summarizes the multi-agent consensus performance.

\begin{table}[h]
\centering
\begin{tabular}{lc}
\toprule
\textbf{Metric} & \textbf{50-Case Evaluation} \\
\midrule
Cases Processed & 50 \\
Processing Rate & $\sim$2 cases/sec (CPU) \\
\midrule
Binary Accuracy & 82.0\% \\
Weighted Precision & 80.9\% \\
Weighted Recall & 82.0\% \\
Weighted F1 Score & 81.4\% \\
\midrule
Majority Agreement & 100\% \\
Split Decisions & 0\% \\
\bottomrule
\end{tabular}
\caption{Multi-agent binary classification results on the 50-case NLMCXR Evaluation Subset.}
\label{tab:results}
\end{table}
...
\midrule
\textbf{Actual Normal} ($n=43$) & 39 (TN) & 4 (FP) \\
\textbf{Actual Abnormal} ($n=7$) & 5 (FN) & 2 (TP) \\
\bottomrule
\end{tabular}
\caption{Binary confusion matrix (Normal vs.\ Abnormal) on the 50-case Evaluation Subset. The dataset is imbalanced: 86\% Normal, 14\% Abnormal.}
\label{tab:confusion}
\end{table}

\subsubsection{Agent Agreement Analysis}

All 50 cases achieved majority agreement (100\%), meaning at least 4 of 6 agents agreed on the binary classification in every case; no split decisions occurred. However, unanimous agreement (all 6 agents producing the same binary prediction) was never observed (0\%). This is expected given the deliberate diversity of agent strategies: the three radiologists operate at different sensitivity thresholds, and the three pathologists use different NLP approaches, so their individual probability outputs naturally vary even when the majority vote converges.

\subsubsection{Confusion Matrix Analysis}

The confusion matrix (Table~\ref{tab:confusion}) reveals important characteristics of the system's behavior on this imbalanced dataset (43 Normal, 7 Abnormal):

\begin{itemize}
    \item \textbf{Normal class specificity:} 39/43 = 90.7\%. The system correctly classifies the vast majority of normal cases, with only 4 false positives.
    \item \textbf{Abnormal class sensitivity:} 2/7 = 28.6\%. The system detects only 2 of 7 abnormal cases, missing 5 (false negatives). This low sensitivity is a concern in a clinical screening context where missed abnormalities carry high cost.
    \item \textbf{Class imbalance effect:} The 86/14 Normal/Abnormal split means the overall 82\% accuracy is largely driven by correct Normal predictions. The weighted F1 score (81.4\%) partially accounts for this by weighting per-class F1 by class frequency.
\end{itemize}
...
The scispaCy medical NER model (\texttt{en\_core\_sci\_sm}) was integrated into Pathologist-2, enabling domain-specific entity recognition. Across the 50-case evaluation, the spaCy agent detected an average of 9.4 medical entities per report, with malignancy scores appropriately adjusted based on entity context (e.g., calcified textures reduced malignancy estimates to 0.10--0.30).
...
\begin{itemize}
    ...
    \item The evaluation on 50 reports with NLP-derived binary ground truth demonstrates pipeline functionality and multi-agent consensus behavior, but larger-scale validation with expert-annotated ground truth would strengthen statistical conclusions.
...
The architecture deploys seven agent instances (three radiologists, three pathologists, one oncologist) that communicate via FIPA-compliant message passing. Evaluation on the 50-case Evaluation Subset from the IU/Open-I NLMCXR dataset achieved 82\% binary classification accuracy with 100\% majority agreement among agents, demonstrating robust consensus convergence. The system exhibited high specificity (90.7\%) for Normal cases but lower sensitivity (28.6\%) for Abnormal cases, reflecting the inherent challenge of detecting rare pathological findings in an imbalanced dataset.
